	...
	def vaccines_weekly(self):
		...
		return {
			'this week': {
				'Dose 1': weekly['dose1'],
				'Dose 2': weekly['dose2'],
				'Week Ending': datetime.strptime(str(records[-1]['WeekEnding']), '%Y%m%d').strftime('%d/%m/%Y')
			},
			'totals': {
				'Dose 1': totals['dose1'],
				'Dose 2': totals['dose2']
	...
	def get_totals(self, records):
		...
		for record in records:
			...
			if record['Dose'] == "Dose 1":
				if record["NumberVaccinated"]: totals['dose1'] += record["NumberVaccinated"]
			elif record['Dose'] == "Dose 2":
				if record["NumberVaccinated"]: totals['dose2'] += record["NumberVaccinated"]
